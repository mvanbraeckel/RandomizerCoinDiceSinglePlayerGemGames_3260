class Item < ApplicationRecord
  ...
  with_options if: :is_coin? do |coin|
    coin.validates :denomination, presence: true, numericality: { only_float: true, greater_than: 0.0 },
      inclusion: { in: -> { [0.05, 0.1, 0.10, 0.25, 1, 1.0, 1.00, 2, 2.0, 2.00].include?(denomination) }, message: "value '%{value}' is not a valid coin denomination" }
